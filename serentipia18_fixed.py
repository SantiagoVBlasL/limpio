...
import subprocess
from models.classifiers12_fixed import get_classifier_and_grid, get_available_classifiers
import joblib
from sklearn.model_selection import StratifiedKFold, train_test_split as sk_train_test_split, RepeatedStratifiedKFold
from sklearn.metrics import roc_auc_score, accuracy_score, recall_score, f1_score, average_precision_score, balanced_accuracy_score
import torch
torch.backends.cudnn.benchmark = True
torch.set_float32_matmul_precision('high') # Ideal para Ampere (RTX 30xx) y superior
...
# Nombre de archivo template para guardar y cargar el modelo VAE por fold
VAE_MODEL_FNAME_TEMPLATE = 'vae_model_fold_{fold}.pt'
...
def load_data(tensor_path: Path, metadata_path: Path) -> Tuple[Optional[np.ndarray], Optional[pd.DataFrame]]:
    ...
    if 'Age_Group' in merged_df.columns:
        # si hay NaNs, marcarlos explícitamente p/ que cuenten como un estrato separado
        merged_df['Age_Group'] = merged_df['Age_Group'].fillna('AgeUnknown').astype(str)
...
# 1) Reemplaza tu callback por esto:
def lgbm_optuna_callback(study: optuna.study.Study, trial: optuna.trial.FrozenTrial) -> None:
    md = trial.params.get("model__max_depth")
    nl = trial.params.get("model__num_leaves")
    if md is None or nl is None:
        return
    # En LGBM, md <= 0 => sin límite (no prunees)
    if md <= 0:
        return
    max_valid_leaves = 2 ** int(md)
    if nl > max_valid_leaves:
        raise optuna.exceptions.TrialPruned(
            f"num_leaves ({nl}) > 2^max_depth ({max_valid_leaves}) para max_depth={md}."
        )
...
def train_and_evaluate_pipeline(global_tensor_all_channels: np.ndarray, 
    ...
    # --- Parche de seguridad ---
    # Algunos scripts que llaman a esta función (p.ej. channel_selection.py) no
    # añaden el atributo `git_hash` al objeto `args`, lo que provocaba:
    #     AttributeError: 'Namespace' object has no attribute 'git_hash'
    # Para garantizar retro‑compatibilidad, añadimos un valor por defecto.
    if not hasattr(args, "git_hash"):
        args.git_hash = "N/A"
    
    # Evita que los workers hija sobre‑usen CPU cores en operaciones BLAS
    import os
    os.environ.setdefault("OMP_NUM_THREADS", "1")
    os.environ.setdefault("MKL_NUM_THREADS", "1")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger.info(f"[pipeline] Usando dispositivo: {device}")
    ...
    for fold_idx, (train_dev_clf_idx_in_cn_ad_df, test_clf_idx_in_cn_ad_df) in enumerate(outer_cv_clf.split(X_classifier_subject_indices_in_cn_ad_df, stratify_key_for_clf_cv)):
        ...
        logger.info(f"{fold_idx_str}: Sujetos CN/AD en entrenamiento/validación: {len(train_dev_clf_idx_in_cn_ad_df)}, Test: {len(test_clf_idx_in_cn_ad_df)}")
        ...
        np.save(fold_output_dir / "subject_ids_test.npy",
                cn_ad_df.iloc[test_clf_idx_in_cn_ad_df]['SubjectID'].values.astype(str))
        np.save(fold_output_dir / "subject_ids_train_dev.npy",
                cn_ad_df.iloc[train_dev_clf_idx_in_cn_ad_df]['SubjectID'].values.astype(str))
        ...
        # Al inicio del procesamiento del fold, después de mapear las etiquetas
        label_mapping = {'CN': 0, 'AD': 1}
        cn_ad_df['label'] = cn_ad_df['ResearchGroup_Mapped'].map(label_mapping)
        positive_label_info = {
            "label_mapping": label_mapping,
            "positive_label_name": "AD",
            "positive_label_int": 1
        }
        # Guardar en el directorio del fold
        import json
        with open(fold_output_dir / "label_mapping.json", 'w') as f:
            json.dump(positive_label_info, f, indent=2)

        logger.info(f"Mapeo de etiquetas guardado en: {fold_output_dir / 'label_mapping.json'}")

        orig_cols = ['ResearchGroup_Mapped', 'Sex', 'Age_Group']
        vae_strat_cols = [c for c in orig_cols if c in temp_vae_strat_df.columns]
        missing = set(orig_cols) - set(vae_strat_cols)
        for m in missing:
            logger.warning(f"Columna '{m}' no encontrada para estratificación VAE. Se omite.")

        try:
            temp_vae_strat_df = vae_train_pool_df[vae_strat_cols].astype(str).copy()
            # Línea correcta
            stratify_key_vae_split = temp_vae_strat_df[vae_strat_cols].apply(lambda x: '_'.join(x.astype(str)), axis=1)
        ...
        if args.vae_val_split_ratio > 0 and len(global_indices_vae_training_pool) > 10:
            try:
                ...
                # BUGFIX: si el split falla, hay que desactivar el scheduler y early stopping para este fold
        ...
        pin_mem = device.type == "cuda" # Performance: pin memory only for GPU
        ...
        # BUGFIX: Si el split de validación ha fallado, vae_internal_val_loader será None.
        use_vae_validation = vae_internal_val_loader is not None
        #device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        vae_fold_k = ConvolutionalVAE(
            ...
            num_conv_layers_encoder=args.num_conv_layers_encoder, decoder_type=args.decoder_type, num_groups=args.gn_num_groups
        ).to(device)
        if device.type == "cuda":
            assert next(vae_fold_k.parameters()).is_cuda, "¡El VAE quedó en CPU!"
            logger.debug(f"[GPU] Modelo en: {torch.cuda.get_device_name(device)}")
        
        optimizer_vae = optim.AdamW(vae_fold_k.parameters(), lr=args.lr_vae, weight_decay=args.weight_decay_vae, amsgrad=True)
        #Performance: Usar torch.compile si está disponible (PyTorch 2.x)
        # Ejecutamos siempre en eager, para evitar errores de compilación en Inductor.
        # vae_fold_k = torch.compile(vae_fold_k)


        scheduler_vae = None
        # ▼▼▼ LÓGICA DE SCHEDULER ACTUALIZADA ▼▼▼
        if use_vae_validation: # Usar la flag que hemos creado
        ...
        if best_model_state_dict:
            ...
            val_string = f"{best_val_loss:.4f}" if use_vae_validation and not np.isnan(best_val_loss) else "N/A - Last Epoch"
        ...
        # Guardar modelo VAE usando el template centralizado
        vae_model_fname = VAE_MODEL_FNAME_TEMPLATE.format(fold=fold_idx+1)
        ...
        if args.save_vae_training_history:
                    ...
                    try:
                        ...
                        if use_vae_validation and any(not np.isnan(x) for x in history_data["val_loss"]):
        ...
        with torch.no_grad():
            ...
            if X_test_final_tensor_norm.shape[0] > 0:
                ...
                # ▼▼▼ LÍNEAS A AÑADIR ▼▼▼
                logger.info("Guardando artefactos adicionales para interpretabilidad...")
                np.save(fold_output_dir / "X_latent_test_mu.npy", mu_test_final.cpu().numpy())
                np.save(fold_output_dir / "X_latent_test_z.npy", z_test_final.cpu().numpy())
                # guarda un CSV ligero con meta por fold
                test_meta = cn_ad_df.iloc[test_clf_idx_in_cn_ad_df][["SubjectID","label","Manufacturer"]]
                test_meta.to_csv(fold_output_dir / "subjects_meta.csv", index=False)
                # Guardar una muestra de los tensores de entrada ya normalizados
                np.save(fold_output_dir / "sample_inputs_norm.npy", X_test_final_tensor_norm[:10])
                # Guardar metadatos clave de la ejecución en un JSON
                selected_channel_indices = args.channels_to_use or list(range(global_tensor_all_channels.shape[1]))

                interpretability_meta = {
                    "vae_hparams": {k: v for k, v in vars(args).items()  # <— quita el filtro, o añádele aquí las keys
                                    if k.startswith(("vae_", "latent_", "num_conv_layers_encoder",
                                                    "decoder_type", "gn_num_groups", "intermediate_fc_dim_vae"))},
                    "channels_used": {
                        "indices": selected_channel_indices,
                        "names": selected_channel_names_in_tensor
                    }
                }
                with open(fold_output_dir / "interpretability_meta.json", "w") as f:
                    json.dump(interpretability_meta, f, indent=2)
        ...
        # 1. Combinar las features latentes con los metadatos CRUDOS en un único DataFrame.
        #    El preprocesamiento (escalado, imputación, one-hot) se hará DENTRO del pipeline.

        # DataFrame de features latentes
        X_latent_train_dev.reset_index(drop=True, inplace=True)
        X_latent_test_final.reset_index(drop=True, inplace=True)
        if args.metadata_features is None:
            args.metadata_features = []          # lista vacía segura



        # DataFrame de metadatos CRUDOS
        metadata_train_dev = clf_train_dev_df[args.metadata_features].copy().reset_index(drop=True)
        metadata_test = cn_ad_df.iloc[test_clf_idx_in_cn_ad_df][args.metadata_features].copy().reset_index(drop=True)

        # Unir para crear los DataFrames finales para el pipeline
        # --- Definir callbacks de Optuna una sola vez ANTES del bucle de clasificadores ---
        optuna_callbacks = []
        if "gb" in args.classifier_types:
            optuna_callbacks.append(lgbm_optuna_callback)
            logger.info("     Guardarraíl para LightGBM (num_leaves <= 2^max_depth) activado.")
        X_train_for_pipeline = pd.concat([X_latent_train_dev, metadata_train_dev], axis=1)
        X_test_for_pipeline = pd.concat([X_latent_test_final, metadata_test], axis=1)

        logger.info(f"Forma de datos de entrenamiento (crudos) para el pipeline: {X_train_for_pipeline.shape}")
        ...
        for current_classifier_type in args.classifier_types:
            ...
            # --- Lógica adaptativa por clasificador ---
            # 1) Desbalance real en este fold
            n_pos = int(np.sum(y_clf_train_dev_all == 1))
            n_neg = int(np.sum(y_clf_train_dev_all == 0))
            imbalance_ratio = max(n_pos, n_neg) / max(1, min(n_pos, n_neg))
            logger.info(f"      Desbalance fold train/dev: pos={n_pos}, neg={n_neg}, ratio={imbalance_ratio:.2f}")

            # REVERTIR A LÓGICA SIMPLE PARA DEPURAR
            use_smote_this = args.use_smote
            balance_this = args.classifier_use_class_weight
            calibrate_this = args.classifier_calibrate
            calibrate_method = 'sigmoid' # O lo que tenías antes, pero simplificado
            ...
            try:
                ...
                full_pipeline, param_distributions, n_iter_search = get_classifier_and_grid(
                    ...
                    latent_dim=args.latent_dim, # <-- Pasar parámetro
                    metadata_cols=args.metadata_features, # <-- Pasar parámetro
                    seed=args.seed,
                    balance=balance_this,
                    use_smote=use_smote_this,
                    tune_sampler_params=args.tune_sampler_params,
                    mlp_hidden_layers=args.mlp_classifier_hidden_layers,
                    calibrate=calibrate_this,
                    calibrate_method=calibrate_method,
                    add_feature_select=(current_classifier_type in ['logreg','svm']),
                    fs_k_min=32,
                    fs_k_max=min(args.latent_dim, 256),
                )
            ...
            # --- CÓDIGO NUEVO (CON PRUNER) ---
            # 1. Crear el sampler que se quiere usar
            sampler = optuna.samplers.TPESampler(seed=args.seed)

            # 2. Opcionalmente, crear un pruner para detener trials no prometedores
            pruner = SuccessiveHalvingPruner() if args.use_optuna_pruner else None
            if pruner:
                logger.info(f"     Usando Optuna pruner: {type(pruner).__name__}")

            # 3. Crear un 'study' de Optuna, especificando la dirección, sampler y pruner.
            #    La dirección debe ser "maximize" porque 'balanced_accuracy' es mejor cuanto más alto.
            ...
            # 1. Capar el número de trials para no exceder un límite razonable (ej. 80)
            max_trials = 80
            effective_n_trials = min(n_iter_search, max_trials)
            #logger.info(f"      Optuna n_trials original: {n_iter_search}. Capado a: {effective_n_trials}.")

            # 2. Definir un timeout en segundos (ej. 1800s = 30 minutos) por clasificador/fold
            timeout_seconds = 1800
            #logger.info(f"      Optuna timeout por búsqueda: {timeout_seconds}s.")
            logger.info(f"      Iniciando OptunaSearchCV con n_trials={effective_n_trials} y timeout={timeout_seconds}s.")

            def _auc_balacc_scorer(estimator, X, y):
                proba = estimator.predict_proba(X)[:,1]
                auc = roc_auc_score(y, proba)
                y_pred = (proba >= 0.5).astype(int)
                bal = balanced_accuracy_score(y, y_pred)
                return 0.7*auc + 0.3*bal
            

            # 3) Ahora sí: crea OptunaSearchCV con callbacks ya definidos
            optuna_search = OptunaSearchCV(
                estimator=full_pipeline,
                param_distributions=param_distributions,
                study=study,
                cv=inner_cv_for_hp_tune,
                scoring=_auc_balacc_scorer,
             ...
             # Llamamos a fit UNA SOLA VEZ
            optuna_search.fit(X_train_for_pipeline, y_clf_train_dev_all)
            final_clf_model = optuna_search.best_estimator_
            
            # --- EVALUACIÓN Y GUARDADO (VERSIÓN FINAL) ---
            fold_results_clf = {
                'fold': fold_idx + 1, 
                'classifier': current_classifier_type,
                'best_params': optuna_search.best_params_
            }

            if not X_test_for_pipeline.empty:
                y_pred_proba = final_clf_model.predict_proba(X_test_for_pipeline)[:, 1]
                y_pred_default = (y_pred_proba >= 0.5).astype(int)

                #from sklearn.model_selection import StratifiedKFold
                from sklearn.base import clone
                skf = StratifiedKFold(n_splits=args.inner_folds, shuffle=True, random_state=args.seed + 777)
                oof_proba = np.zeros_like(y_clf_train_dev_all, dtype=float)
                for tr, va in skf.split(X_train_for_pipeline, y_clf_train_dev_all):
                    est = clone(final_clf_model)
                    est.fit(X_train_for_pipeline.iloc[tr], y_clf_train_dev_all[tr])
                    oof_proba[va] = est.predict_proba(X_train_for_pipeline.iloc[va])[:,1]
                fpr, tpr, thresholds = roc_curve(y_clf_train_dev_all, oof_proba)
                optimal_threshold = thresholds[np.argmax(tpr - fpr)]
                y_pred_optimal = (y_pred_proba >= optimal_threshold).astype(int)
                
                logger.info(f"      Umbral óptimo (Youden's J): {optimal_threshold:.4f}")
                ...
                fold_results_clf.update({
                    ...
                    'acc_def': accuracy_score(y_test_final, y_pred_default),
                    'bal_acc_def': balanced_accuracy_score(y_test_final, y_pred_default),
                    'sens_def': recall_score(y_test_final, y_pred_default, pos_label=1, zero_division=0),
                    'spec_def': recall_score(y_test_final, y_pred_default, pos_label=0, zero_division=0),
                    'f1_def': f1_score(y_test_final, y_pred_default, pos_label=1, zero_division=0),
                    'acc_opt': accuracy_score(y_test_final, y_pred_optimal),
                    'bal_acc_opt': balanced_accuracy_score(y_test_final, y_pred_optimal),
                    'sens_opt': recall_score(y_test_final, y_pred_optimal, pos_label=1, zero_division=0),
                    'spec_opt': recall_score(y_test_final, y_pred_optimal, pos_label=0, zero_division=0),
                    'f1_opt': f1_score(y_test_final, y_pred_optimal, pos_label=1, zero_division=0),
                })
                
                log_msg = (
                    f"      Resultados: AUC={fold_results_clf['auc']:.4f} | "
                    f"Bal.Acc (def)={fold_results_clf['bal_acc_def']:.4f} -> "
                    f"Bal.Acc (opt)={fold_results_clf['bal_acc_opt']:.4f}"
                )
                logger.info(log_msg)
                # ⓹ Añadir la métrica custom definida por --gridsearch_scoring para evitar KeyError en pasos posteriores
                # --- INICIO: Bloque para imprimir hiperparámetros ---
                logger.info(f"      Mejores hiperparámetros encontrados para {current_classifier_type} en {fold_idx_str}:")
                best_params = optuna_search.best_params_
            ...
            # La lógica de SHAP ahora se ejecuta después del único fit, si es 'gb'
            if current_classifier_type == "gb":
                 preprocessor = final_clf_model.named_steps['preprocessor']
                 n_bg = min(100, len(X_train_for_pipeline))
                 background_raw = X_train_for_pipeline.sample(n=n_bg, random_state=args.seed) if n_bg > 0 else X_train_for_pipeline
                 background_processed = preprocessor.transform(background_raw)
                 background_df = pd.DataFrame(background_processed, columns=preprocessor.get_feature_names_out())
 
                 # Guardar artefactos para SHAP
                 joblib.dump(background_df, fold_output_dir / f"shap_background_data_{current_classifier_type}.joblib")
                 joblib.dump(background_raw, fold_output_dir / f"shap_background_raw_{current_classifier_type}.joblib")
                 with open(fold_output_dir / f"metadata_cols_{current_classifier_type}.txt", "w") as f:
                     f.write("\n".join(args.metadata_features))
                 logger.info("Background data para SHAP guardado.")
            ...
            if args.save_fold_artefacts:
                ...
                logger.info(f"      Pipeline completo de {current_classifier_type} guardado.")
        ...
        if logger.getEffectiveLevel() == logging.DEBUG and device.type == 'cuda':
            logger.debug(f"GPU Memory Summary (end of fold {fold_idx + 1}):\n{torch.cuda.memory_summary()}")
    ...
    if all_folds_metrics:
        ...
        for clf_type_iterated in args.classifier_types:
            metrics_df_clf = metrics_df[metrics_df['classifier'] == clf_type_iterated]
            if not metrics_df_clf.empty:
                logger.info(f"\n--- Resumen de Rendimiento para Clasificador: {clf_type_iterated} (Promedio sobre Folds Externos) ---")
                
                # Bucle dinámico sobre todas las columnas de métricas
                metric_cols = [c for c in metrics_df_clf.columns if c not in ['fold', 'classifier', 'best_params']]
                for metric in sorted(metric_cols):
                    if metrics_df_clf[metric].notna().any():
                        mean_val = metrics_df_clf[metric].mean()
                        std_val = metrics_df_clf[metric].std()
                        logger.info(f"{metric.replace('_', ' ').capitalize():<30}: {mean_val:.4f} +/- {std_val:.4f}")
        ...
        # Solo escribimos el hash si existe (tras el parche anterior debería).
            if hasattr(args, "git_hash"):
                f.write(f"Git Commit Hash: {args.git_hash}\n\n")
            for clf_type_iterated in args.classifier_types:
                metrics_df_clf = metrics_df[metrics_df['classifier'] == clf_type_iterated]
                if not metrics_df_clf.empty:
                    f.write(f"--- Metrics Summary for Classifier: {clf_type_iterated} ---\n")
                    skip = {'fold', 'classifier', 'best_params'}
                    numeric_cols = [c for c in metrics_df_clf.columns
                                    if c not in skip and metrics_df_clf[c].dtype.kind in "fi"]
                    for metric in sorted(numeric_cols):
                        f.write(f"{metric:<20}: {metrics_df_clf[metric].mean():.4f} +/- {metrics_df_clf[metric].std():.4f}\n")
        ...
        # --- Bloque Adicional: Análisis de Hiperparámetros del Mejor Modelo ---
        logger.info("\n--- Análisis de Hiperparámetros del Mejor Clasificador ---")
        # Determinar el mejor clasificador basado en la métrica de scoring principal
        summary_by_clf = metrics_df.groupby('classifier')['auc'].mean()
        best_clf_type = summary_by_clf.idxmax()
        logger.info(f"Mejor clasificador según el promedio de 'auc': {best_clf_type} ({summary_by_clf.max():.4f})")
        # Filtrar las métricas del mejor clasificador
        best_clf_metrics = metrics_df[metrics_df['classifier'] == best_clf_type]
        logger.info("Mejores hiperparámetros encontrados por fold para este clasificador:")
        for _, row in best_clf_metrics.iterrows():
            logger.info(f"  Fold {row['fold']}:")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Pipeline VAE+Clasificador para AD/CN (v1.9 Final)",
    ...
    group_data.add_argument(
        ...
        nargs="*",
        default=None,
        help="Índices de canales a usar (p. ej. 1 2 5)."
    )
    ...
    group_data.add_argument("--output_dir", type=str, default="./vae_clf_output", help="Directorio para guardar resultados.")
    #group_data.add_argument("--output_dir", type=str, default="./vae_clf_output_v1.8.1", help="Directorio para guardar resultados.")
    group_data.add_argument(
                                "--roi_order_path",
                                type=str,
                                required=True,
    ...
    group_vae.add_argument("--dropout_rate_vae", type=float, default=0.25, help="Tasa de dropout en VAE.")
    group_vae.add_argument("--use_layernorm_vae_fc", action='store_true', help="Usar LayerNorm en capas FC del VAE.")
    ## --- INICIO CORRECCIÓN 2: AÑADIR ARGUMENTO PARA NUM_GROUPS ---
    group_vae.add_argument("--gn_num_groups", type=int, default=16, help="Número de grupos para GroupNorm en el VAE. (Recomendado: 8, 16, 32)")
    ## --- FIN CORRECCIÓN 2 ---
    ...
    # Unificamos el control de logs en un solo argumento
    group_general.add_argument("--log-level", dest="log_level", type=str, default="INFO", choices=["DEBUG", "INFO", "WARNING", "ERROR"],
                                help="Nivel de verbosidad del logger.")
 

    args = parser.parse_args()
    # Parche para evitar TypeError si no se pasan metadatos
    if args.metadata_features is None:
        args.metadata_features = []
    ...
    # Configurar el nivel del logger principal
    logging.getLogger("__main__").setLevel(args.log_level.upper())
    ...
    logger.info("--- Configuración de la Ejecución (v1.9 Final) ---")
